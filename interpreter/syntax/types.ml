...
let conttype_of_comptype = function ContT ut -> ut | _ -> assert false
...
let as_def_heaptype = function UseHT (Def dt) -> dt | _ -> assert false

let as_cont_func_heaptype (ht : heaptype) : (resulttype * resulttype) =
  let ut = conttype_of_comptype (expand_deftype (as_def_heaptype ht)) in
  functype_of_comptype (expand_deftype (deftype_of_typeuse ut))

let as_cont_func_reftype (rt : valtype) : (resulttype * resulttype) =
  match rt with
  | RefT (_, ht) -> as_cont_func_heaptype ht
  | _ -> assert false
