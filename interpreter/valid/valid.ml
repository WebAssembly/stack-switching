...
let rec check_instr (c : context) (e : instr) (s : infer_resulttype) : infer_instrtype =
  ...
  | ContBind (x, y) ->
    ...
    let (ts1', ts2') = func_type_of_cont_type c ct' y.at in
  ...
  | Switch (x, y) ->
     ...
     let (ts11, ts12) = func_type_of_cont_type c ct1 x.at in
     let (ts21, ts22) =
       match Lib.List.last_opt ts11 with
       | Some (RefT (nul', ht)) ->
         func_type_of_cont_type c (cont_type_of_heaptype c ht x.at) x.at
       | _ ->
         error x.at
           ("type mismatch: instruction requires continuation reference type" ^
              " but the type annotation has " ^ string_of_resulttype ts11)
     in
     let (ts31, t) = func_type_of_tag_type c (tag c y) y.at in
     require (ts31 = []) y.at
...
and check_catch (c : context) (cc : catch) (ts : valtype list) at =
  ...
  match cc.it with
  | Catch (x1, x2) ->
    let (ts1, ts2) = func_type_of_tag_type c (tag c x1) x1.at in
